...
FORBIDDEN_CONFIG = {
    "ex02_max_in_list": {
        "calls": {"max"},
        "methods": set(),      # attribute .sort() not forbidden here
        "in_operator": False,
    },
    "ex04_selection_sort": {
        "calls": {"sorted"},   # forbid global sorted()
        "methods": {"sort"},   # forbid .sort()
        "in_operator": False,
    },
    "ex05_binary_search": {
        "calls": {"sorted"},
        "methods": {"sort"},
        # binary search should not be implemented with "if target in values"
        "in_operator": True,
    },
    # You can add more rules later if needed
